...
def generate_new_sample(template, htmlgrammar, cssgrammar, jsgrammar, g=0, fi=''):
    ...
    fi = rreplace(fi, str(g-1), str(g), 1 )                                                      #fi.replace(str(g-1),str(g))
    result = template
...
def generate_samples(grammar_dir, outfiles):
    ...
    for outfile in outfiles:
        ...
        result = generate_new_sample(template, htmlgrammar, cssgrammar,
                                     jsgrammar, g=ttt, fi=outfile) #F:/domato/curpus/
...
def main():
    ...
    def createfile(n):
        return runcommand("touch "+n)
    
    def createdir(n):
        return runcommand("mkdir "+n)
    ...
    if multiple_samples:
        ...
        out_dir = os.path.abspath(get_option('--output_dir'))
        if not checkdir(out_dir):
            b = createdir(out_dir)
            if not b:
